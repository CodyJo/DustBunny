...
function parseRoutingFlags(argv) {
  ...
  for (const arg of argv) {
    if (arg === '--prefer-official') {
      routing.preferOfficial = true;
      continue;
    }
    if (arg === '--prefer-native') {
      routing.preferNative = true;
      continue;
    }
    if (arg === '--no-fallback') {
...
async function runCli(argv = process.argv.slice(2), options = {}) {
  ...
  const officialPassthrough = !(options.disableOfficialPassthrough || options.disableLegacyPassthrough || routing.preferNative)
    ? buildOfficialBunnyArgs(argv)
    : null;
  ...
  if (officialPassthrough) {
    ...
    if (!officialPassthrough.fallbackToCustom || routing.noFallback) {
      if (result?.error) {
        fail(`Official Bunny CLI failed for ${argv.join(' ')}: ${result.error.message}`);
      }
      return result?.code ?? 1;
    }
    stderr.write(`DustBunny: official Bunny CLI failed for "${argv.join(' ')}"; falling back to native implementation.\n`);
  }
